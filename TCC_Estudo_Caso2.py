...
def calculo_producao_total(num_turbinas, vet_layout_x, vet_yaw, wind_speeds, wind_directions, fi):
    ...
    for i in range(num_casos):
        ...
        yaw_angles = np.zeros((1, 1, 1))  # Inicializando yaw_angles
        ...
        if(plot_wake==True):
            ...
            title = f"Distância entre T: {vet_layout_x[i]}m - Ângulo: {vet_yaw_nom[i]}° - Produção: {farm_powers[i]:.2f}kW"
...
increment = 20
vet_layout_x = np.arange(start, end + increment, increment)

# Crie o vetor de ângulos usando np.arange
vet_yaw_nom = np.zeros((20,num_turbine))
...
# Crie o gráfico
plt.figure(figsize=(10, 6))
plt.plot(vet_layout_x, farm_powers_nom, marker='o', linestyle='-', color='b', label='Farm Powers Nom')
plt.plot(vet_layout_x, farm_t[0, :], marker='o', linestyle='-', color='r', label='T1')
plt.plot(vet_layout_x, farm_t[1, :], marker='o', linestyle='-', color='g', label='T2')

plt.title('Potência X Distância (x)')
